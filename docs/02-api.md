...
| 항목 | 내용 |
|---|---|
| 설명 | 특정 공간의 모든 좌석 배치와 선택한 KST 날짜·시간 기준 상태를 반환. 클라이언트는 이 응답으로 좌석 그리드를 렌더링. |
| 인증 | 불필요 |
...
| 이름 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `at` | string(ISO8601 datetime) | X | 선택한 날짜·시간 스냅샷. 오프셋 생략 시 KST, 미전달 시 현재 KST 시각 |

**미래 날짜 조회 예시**

```http
GET /api/spaces/{id}/seats?at=2026-07-20T09:00:00%2B09:00
```

> URL 쿼리에서 `+09:00`의 `+`는 `%2B`로 인코딩한다.
> 오프셋 없는 `2026-07-20T09:00:00` 형식도 KST로 해석한다.
...
| 항목 | 내용 |
|---|---|
| 설명 | 특정 좌석의 위치, 선택한 KST 날짜·시간 기준 상태, 소속 공간 정보를 조회한다. |
| 인증 | 불필요 |
...
```http
GET /api/seats/{id}?at=2026-07-20T09:00:00%2B09:00
```
